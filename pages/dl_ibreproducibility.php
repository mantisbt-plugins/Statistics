...
foreach ( $result as $row ) {
    $data[$row['reproducibility']][$row['status']] = $row['the_count'];
}
unset( $result );
...
$content .= "\r\n";
// build header row
$content .= lang_get( 'reproducibility' ) ;
		$content .= "|" ;
...
//		break; 
	...
	$content .=  MantisEnum::getLabel( $reproducibility_enum_string, $key ) ;
	$content .= "|" ;
    foreach ( $status_values as $k => $v ) {
...
header("Content-Disposition: attachment; filename=Issues_By_Reproducibility.csv");
echo $content;
exit;
return;